...
export default new ChatInput(
  {
    name: 'wink',
    description: 'Faire un clin d’œil avec un GIF',
    defaultMemberPermissions: null,
    dmPermission: true,
  },
  async (interaction) => {
    try {
      const response = await axios.get('https://nekos.best/api/v2/wink');
      if (response.data.results.length === 0) {
        return interaction.reply({ content: 'Aucune image disponible pour cette interaction.', ephemeral: true });
      }

      const imageUrl = response.data.results[0].url;
      const fileName = `wink-${Date.now()}.gif`;

      // Télécharger l'image
      const imageResponse = await axios.get(imageUrl, { responseType: 'arraybuffer' });
      fs.writeFileSync(fileName, imageResponse.data);

      const actionText = `${interaction.user} fait un clin d’œil <:zrougelovii:1260653164487770193>`;
      ...
      console.error('Erreur lors de l’interaction "wink":', error);
