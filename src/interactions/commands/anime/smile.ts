...
export default new ChatInput(
  {
    name: 'smile',
    description: 'Sourire avec un GIF chaleureux',
  ...
  async (interaction) => {
    try {
      ...
      const fileName = `smile-${Date.now()}.gif`;

      // Télécharger l'image
      const imageResponse = await axios.get(imageUrl, { responseType: 'arraybuffer' });
      fs.writeFileSync(fileName, imageResponse.data);

      const actionText = `${interaction.user} sourit chaleureusement <:zrougelovii:1260653164487770193>`;
      ...
      console.error('Erreur lors de l’interaction "smile":', error);
