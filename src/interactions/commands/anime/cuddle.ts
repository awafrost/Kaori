...
export default new ChatInput(
  {
    name: 'cuddle',
    description: 'Faire un câlin ou se câliner avec un GIF',
    options: [
      {
        name: 'cible',
  ...
  async (interaction) => {
    ...
    const actionText = targetUser
      ? `${interaction.user} fait un câlin à ${targetUser} <:zrougelovii:1260653164487770193>`
      : `${interaction.user} se fait un câlin tout seul <:zrougelovii:1260653164487770193>`;
    ...
    try {
      ...
      const fileName = `cuddle-${Date.now()}.gif`;
      ...
      console.error('Erreur lors de l’interaction "cuddle":', error);
