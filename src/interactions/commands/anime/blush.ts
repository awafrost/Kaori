...
export default new ChatInput(
  {
    name: 'blush',
    description: 'Rougir avec un GIF adorable',
  ...
  async (interaction) => {
    try {
      ...
      const fileName = `blush-${Date.now()}.gif`;

      // Télécharger l'image
      const imageResponse = await axios.get(imageUrl, { responseType: 'arraybuffer' });
      fs.writeFileSync(fileName, imageResponse.data);

      const actionText = `${interaction.user} rougit adorablement <:zrougelovii:1260653164487770193>`;
      ...
      console.error('Erreur lors de l’interaction "blush":', error);
