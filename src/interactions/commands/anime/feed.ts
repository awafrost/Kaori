...
export default new ChatInput(
  {
    name: 'feed',
    description: 'Nourrir quelqu’un ou se nourrir avec un GIF',
    options: [
      {
        name: 'cible',
        description: 'L’utilisateur à nourrir',
  ...
  async (interaction) => {
    ...
    const actionText = targetUser
      ? `${interaction.user} nourrit ${targetUser} <:zrougelovii:1260653164487770193>`
      : `${interaction.user} se nourrit tout seul <:zrougelovii:1260653164487770193>`;
    ...
    try {
      ...
      const fileName = `feed-${Date.now()}.gif`;
      ...
      console.error('Erreur lors de l’interaction "feed":', error);
