...
export default new ChatInput(
  {
    name: 'pout',
    description: 'Faire la moue avec un GIF',
  ...
  async (interaction) => {
    try {
      ...
      const fileName = `pout-${Date.now()}.gif`;

      // Télécharger l'image
      const imageResponse = await axios.get(imageUrl, { responseType: 'arraybuffer' });
      fs.writeFileSync(fileName, imageResponse.data);

      const actionText = `${interaction.user} fait la moue <:zrougelovii:1260653164487770193>`;
      ...
      console.error('Erreur lors de l’interaction "pout":', error);
