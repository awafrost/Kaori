...
export default new ChatInput(
  {
    name: 'sleep',
    description: 'Dormir avec un GIF paisible',
  ...
  async (interaction) => {
    try {
      ...
      const fileName = `sleep-${Date.now()}.gif`;

      // Télécharger l'image
      const imageResponse = await axios.get(imageUrl, { responseType: 'arraybuffer' });
      fs.writeFileSync(fileName, imageResponse.data);

      const actionText = `${interaction.user} dort paisiblement <:zrougelovii:1260653164487770193>`;
      ...
      console.error('Erreur lors de l’interaction "sleep":', error);
