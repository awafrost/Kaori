...
export default new ChatInput(
  {
    name: 'nope',
    description: 'Dire non avec un GIF',
  ...
  async (interaction) => {
    try {
      ...
      const fileName = `nope-${Date.now()}.gif`;

      // Télécharger l'image
      const imageResponse = await axios.get(imageUrl, { responseType: 'arraybuffer' });
      fs.writeFileSync(fileName, imageResponse.data);

      const actionText = `${interaction.user} dit non avec assurance <:zrougelovii:1260653164487770193>`;
      ...
      console.error('Erreur lors de l’interaction "nope":', error);
