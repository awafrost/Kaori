...
export default new ChatInput(
  {
    name: 'poke',
    description: 'Taquiner quelqu’un ou soi-même avec un GIF',
    options: [
      {
        name: 'cible',
        description: 'L’utilisateur à taquiner',
  ...
  async (interaction) => {
    ...
    const actionText = targetUser
      ? `${interaction.user} taquine ${targetUser} <:zrougelovii:1260653164487770193>`
      : `${interaction.user} se taquine lui-même <:zrougelovii:1260653164487770193>`;
    ...
    try {
      ...
      const fileName = `poke-${Date.now()}.gif`;
      ...
      console.error('Erreur lors de l’interaction "poke":', error);
