...
export default new ChatInput(
  {
    name: 'bite',
    description: 'Mordre quelqu’un ou jouer à mordre avec un GIF',
    options: [
      {
        name: 'cible',
        description: 'L’utilisateur à mordre',
  ...
  async (interaction) => {
    ...
    const actionText = targetUser
      ? `${interaction.user} mord ${targetUser} <:zrougelovii:1260653164487770193>`
      : `${interaction.user} mord dans l’air <:zrougelovii:1260653164487770193>`;
    ...
    try {
      ...
      const fileName = `bite-${Date.now()}.gif`;
      ...
      console.error('Erreur lors de l’interaction "bite":', error);
