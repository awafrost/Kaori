...
export default new ChatInput(
  {
    name: 'dance',
    description: 'Danser avec un GIF énergique',
  ...
  async (interaction) => {
    try {
      ...
      const fileName = `dance-${Date.now()}.gif`;

      // Télécharger l'image
      const imageResponse = await axios.get(imageUrl, { responseType: 'arraybuffer' });
      fs.writeFileSync(fileName, imageResponse.data);

      const actionText = `${interaction.user} danse avec énergie <:zrougelovii:1260653164487770193>`;
      ...
      console.error('Erreur lors de l’interaction "dance":', error);
