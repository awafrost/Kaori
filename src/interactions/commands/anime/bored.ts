...
export default new ChatInput(
  {
    name: 'bored',
    description: 'Montrer son ennui avec un GIF',
  ...
  async (interaction) => {
    try {
      ...
      const fileName = `bored-${Date.now()}.gif`;

      // Télécharger l'image
      const imageResponse = await axios.get(imageUrl, { responseType: 'arraybuffer' });
      fs.writeFileSync(fileName, imageResponse.data);

      const actionText = `${interaction.user} s’ennuie profondément <:zrougelovii:1260653164487770193>`;
      ...
      console.error('Erreur lors de l’interaction "bored":', error);
