...
export default new ChatInput(
  {
    name: 'slap',
    description: 'Giflez un autre utilisateur',
    options: [
      {
        name: 'cible',
  ...
  async (interaction) => {
    ...
    try {
      ...
      const fileName = `slap-${Date.now()}.gif`;

      // Télécharger l'image
      const imageResponse = await axios.get(imageUrl, { responseType: 'arraybuffer' });
      fs.writeFileSync(fileName, imageResponse.data);

      const actionText = `${interaction.user} a giflé ${targetUser} <:zrougelovii:1260653164487770193>`;
      ...
      console.error('Erreur lors de l’interaction "slap":', error);
