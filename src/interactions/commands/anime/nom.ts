...
export default new ChatInput(
  {
    name: 'nom',
    description: 'Manger ou grignoter quelque chose avec un GIF',
    options: [
      {
        name: 'cible',
        description: 'L’utilisateur à grignoter',
  ...
  async (interaction) => {
    ...
    const actionText = targetUser
      ? `${interaction.user} grignote ${targetUser} <:zrougelovii:1260653164487770193>`
      : `${interaction.user} mange quelque chose <:zrougelovii:1260653164487770193>`;
    ...
    try {
      ...
      const fileName = `nom-${Date.now()}.gif`;
      ...
      console.error('Erreur lors de l’interaction "nom":', error);
