...
export default new ChatInput(
  {
    name: 'facepalm',
    description: 'Faire un facepalm avec un GIF',
  ...
  async (interaction) => {
    try {
      ...
      const fileName = `facepalm-${Date.now()}.gif`;

      // Télécharger l'image
      const imageResponse = await axios.get(imageUrl, { responseType: 'arraybuffer' });
      fs.writeFileSync(fileName, imageResponse.data);

      const actionText = `${interaction.user} fait un facepalm <:zrougelovii:1260653164487770193>`;
      ...
      console.error('Erreur lors de l’interaction "facepalm":', error);
