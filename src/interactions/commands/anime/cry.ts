...
export default new ChatInput(
  {
    name: 'cry',
    description: 'Pleurer avec un GIF émouvant',
  ...
  async (interaction) => {
    try {
      ...
      const fileName = `cry-${Date.now()}.gif`;

      // Télécharger l'image
      const imageResponse = await axios.get(imageUrl, { responseType: 'arraybuffer' });
      fs.writeFileSync(fileName, imageResponse.data);

      const actionText = `${interaction.user} pleure à chaudes larmes <:zrougelovii:1260653164487770193>`;
      ...
      console.error('Erreur lors de l’interaction "cry":', error);
