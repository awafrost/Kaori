...
export default new ChatInput(
  {
    name: 'think',
    description: 'Réfléchir avec un GIF',
  ...
  async (interaction) => {
    try {
      ...
      const fileName = `think-${Date.now()}.gif`;

      // Télécharger l'image
      const imageResponse = await axios.get(imageUrl, { responseType: 'arraybuffer' });
      fs.writeFileSync(fileName, imageResponse.data);

      const actionText = `${interaction.user} réfléchit intensément <:zrougelovii:1260653164487770193>`;
      ...
      console.error('Erreur lors de l’interaction "think":', error);
