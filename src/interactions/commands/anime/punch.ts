...
export default new ChatInput(
  {
    name: 'punch',
    description: 'Donner un coup ou simuler un punch avec un GIF',
    options: [
      {
        name: 'cible',
  ...
  async (interaction) => {
    ...
    const actionText = targetUser
      ? `${interaction.user} donne un coup à ${targetUser} <:zrougelovii:1260653164487770193>`
      : `${interaction.user} donne un coup dans le vide <:zrougelovii:1260653164487770193>`;
    ...
    try {
      ...
      const fileName = `punch-${Date.now()}.gif`;
      ...
      console.error('Erreur lors de l’interaction "punch":', error);
