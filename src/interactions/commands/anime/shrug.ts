...
export default new ChatInput(
  {
    name: 'shrug',
    description: 'Hauser les épaules avec un GIF',
  ...
  async (interaction) => {
    try {
      ...
      const fileName = `shrug-${Date.now()}.gif`;

      // Télécharger l'image
      const imageResponse = await axios.get(imageUrl, { responseType: 'arraybuffer' });
      fs.writeFileSync(fileName, imageResponse.data);

      const actionText = `${interaction.user} hausse les épaules <:zrougelovii:1260653164487770193>`;
      ...
      console.error('Erreur lors de l’interaction "shrug":', error);
