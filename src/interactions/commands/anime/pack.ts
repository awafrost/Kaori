...
export default new ChatInput(
  {
    name: 'peck',
    description: 'Donner un petit bisou ou en simuler un avec un GIF',
    options: [
      {
        name: 'cible',
        description: 'L’utilisateur à embrasser légèrement',
  ...
  async (interaction) => {
    ...
    const actionText = targetUser
      ? `${interaction.user} donne un petit bisou à ${targetUser} <:zrougelovii:1260653164487770193>`
      : `${interaction.user} fait un petit bisou dans l’air <:zrougelovii:1260653164487770193>`;
    ...
    try {
      ...
      const fileName = `peck-${Date.now()}.gif`;
      ...
      console.error('Erreur lors de l’interaction "peck":', error);
