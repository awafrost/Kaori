...
export default new ChatInput(
  {
    name: 'lurk',
    description: 'Observer discrètement avec un GIF',
  ...
  async (interaction) => {
    try {
      ...
      const fileName = `lurk-${Date.now()}.gif`;

      // Télécharger l'image
      const imageResponse = await axios.get(imageUrl, { responseType: 'arraybuffer' });
      fs.writeFileSync(fileName, imageResponse.data);

      const actionText = `${interaction.user} observe discrètement <:zrougelovii:1260653164487770193>`;
      ...
      console.error('Erreur lors de l’interaction "lurk":', error);
