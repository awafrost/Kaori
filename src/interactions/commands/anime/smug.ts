...
export default new ChatInput(
  {
    name: 'smug',
    description: 'Afficher un air suffisant avec un GIF',
  ...
  async (interaction) => {
    try {
      ...
      const fileName = `smug-${Date.now()}.gif`;

      // Télécharger l'image
      const imageResponse = await axios.get(imageUrl, { responseType: 'arraybuffer' });
      fs.writeFileSync(fileName, imageResponse.data);

      const actionText = `${interaction.user} affiche un air suffisant <:zrougelovii:1260653164487770193>`;
      ...
      console.error('Erreur lors de l’interaction "smug":', error);
