...
export default new ChatInput(
  {
    name: 'shoot',
    description: 'Tirer sur quelqu’un ou faire semblant avec un GIF',
    options: [
      {
        name: 'cible',
        description: 'L’utilisateur sur qui tirer',
  ...
  async (interaction) => {
    ...
    const actionText = targetUser
      ? `${interaction.user} tire sur ${targetUser} <:zrougelovii:1260653164487770193>`
      : `${interaction.user} tire dans l’air <:zrougelovii:1260653164487770193>`;
    ...
    try {
      ...
      const fileName = `shoot-${Date.now()}.gif`;
      ...
      console.error('Erreur lors de l’interaction "shoot":', error);
