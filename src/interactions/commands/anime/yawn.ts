...
export default new ChatInput(
  {
    name: 'yawn',
    description: 'Bâiller avec un GIF',
  ...
  async (interaction) => {
    try {
      ...
      const fileName = `yawn-${Date.now()}.gif`;

      // Télécharger l'image
      const imageResponse = await axios.get(imageUrl, { responseType: 'arraybuffer' });
      fs.writeFileSync(fileName, imageResponse.data);

      const actionText = `${interaction.user} bâille bruyamment <:zrougelovii:1260653164487770193>`;
      ...
      console.error('Erreur lors de l’interaction "yawn":', error);
