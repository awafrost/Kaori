...
export default new ChatInput(
  {
    name: 'nod',
    description: 'Hocher la tête avec un GIF',
  ...
  async (interaction) => {
    try {
      ...
      const fileName = `nod-${Date.now()}.gif`;

      // Télécharger l'image
      const imageResponse = await axios.get(imageUrl, { responseType: 'arraybuffer' });
      fs.writeFileSync(fileName, imageResponse.data);

      const actionText = `${interaction.user} hoche la tête <:zrougelovii:1260653164487770193>`;
      ...
      console.error('Erreur lors de l’interaction "nod":', error);
