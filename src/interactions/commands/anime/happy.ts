...
export default new ChatInput(
  {
    name: 'happy',
    description: 'Montrer sa joie avec un GIF',
  ...
  async (interaction) => {
    try {
      ...
      const fileName = `happy-${Date.now()}.gif`;

      // Télécharger l'image
      const imageResponse = await axios.get(imageUrl, { responseType: 'arraybuffer' });
      fs.writeFileSync(fileName, imageResponse.data);

      const actionText = `${interaction.user} rayonne de bonheur <:zrougelovii:1260653164487770193>`;
      ...
      console.error('Erreur lors de l’interaction "happy":', error);
