...
export default new ChatInput(
  {
    name: 'laugh',
    description: 'Rire avec un GIF hilarant',
  ...
  async (interaction) => {
    try {
      ...
      const fileName = `laugh-${Date.now()}.gif`;

      // Télécharger l'image
      const imageResponse = await axios.get(imageUrl, { responseType: 'arraybuffer' });
      fs.writeFileSync(fileName, imageResponse.data);

      const actionText = `${interaction.user} rit aux éclats <:zrougelovii:1260653164487770193>`;
      ...
      console.error('Erreur lors de l’interaction "laugh":', error);
